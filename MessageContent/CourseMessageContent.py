...
class CourseMessageContent(MessageContent):
    ...
    def get_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            title=title, description=self.description, url=url, color=self.color
        )
        ...
        if (
            self.deg_attr is not None
        ):  # TODO add to api, API doesnt currently support this for some reason
            embed.add_field(name="Degree Attributes", value=self.deg_attr, inline=False)

        embed.add_field(
            name="Most Recently Offered In", value=self.status, inline=False
        )

        if random.random() < 0.25:
            embed.set_footer(
                text=random.choice(mistakes)
                + " Make an issue on GitHub (timot3/uiuc-classes-bot)."
            )
        elif random.random() < 0.5:
            embed.set_footer(
                text="This project is run entirely by one student, and would not be available without "
